...
class AdminDashboard(MDScreen):
        ...
        try:
            ...
            try:
                ...
                self.audit_logger = AuditLogger(
                    dynamo_manager=self.dynamo_manager, db_manager=self.db_manager
                )
                ...
                Logger.info(
                    "All managers initialized successfully with DynamoDB integration"
                )
                return
    ...
    def _verify_dynamo_connection(self):
        ...
        try:
            if self.dynamo_manager:
                ...
                Logger.info(
                    f"DynamoDB tables verified: Users: {users_table}, Permissions: {permissions_table}, Sessions: {sessions_table}"
                )
                return True
        except Exception as e:
    ...
    def show_create_folder_dialog(self, *args):
        ...
        self.folder_name_input = MDTextField(
            hint_text="Folder Name", mode="outlined", size_hint_y=None, height=dp(48)
        )
        ...
        buttons = MDBoxLayout(
            orientation="horizontal", spacing=dp(16), size_hint_y=None, height=dp(50)
        )

        # Cancel button
        cancel_button = MDButton(style="text", on_release=self._on_cancel_folder)
    ...
    def _create_folder_directly(self, folder_name, user_id=None):
        ...
        try:
            ...
            Logger.info(
                f"Creating empty object with key: {folder_name} in bucket: {bucket_name}"
            )

            # Create folder (empty object with trailing slash)
            response = s3_client.put_object(
                Bucket=bucket_name, Key=folder_name, Body=b""
            )
            ...
            if folder_created:
                ...
                self.show_snackbar(
                    f"Folder '{folder_name.rstrip('/')}' created successfully"
                )
    ...
    async def _update_user_directly_in_dynamo(
        self, username: str, updates: Dict
    ) -> Dict:
        """Update user directly in DynamoDB with fix for serialization issue"""
        try:
            Logger.info(
                f"DIRECT DYNAMO UPDATE: Updating user {username} with updates: {updates}"
            )
            ...
            try:
                ...
                if "folder_access" in updates:
                    if isinstance(updates["folder_access"], list):
                        ...
                        updates["folder_access"] = [
                            str(folder) for folder in updates["folder_access"]
                        ]
                    elif isinstance(updates["folder_access"], str):
                        updates["folder_access"] = [
                            f.strip()
                            for f in updates["folder_access"].split(",")
                            if f.strip()
                        ]
                ...
                end_time = time.time()
                Logger.info(
                    f"DynamoDB update completed in {end_time - start_time:.2f} seconds"
                )
    ...
    def _direct_dynamo_update(self, username, updates):
        ...
        try:
            ...
            if "folder_access" in updates and isinstance(
                updates["folder_access"], list
            ):
                updates["folder_access"] = [
                    str(folder) for folder in updates["folder_access"]
                ]
            ...
            # Log the operation
            Logger.info(
                f"Update operation: Table={users_table_name}, Key={{username={username}, sk=#USER}}"
            )
            ...
            if response and "Attributes" in response:
                ...
                return {
                    "success": False,
                    "error": "Update operation did not return updated attributes",
                }
                    ...
                    if hasattr(self, "dialog") and self.dialog:
                        ...
                        try:
                            ...
                            for child in self.dialog.content.children:
                                if (
                                    isinstance(child, MDLabel)
                                    and username in child.text
                                ):
                                    title_widget = child
                                    break
                            ...
                            if title_widget:
                                ...
                                Clock.schedule_once(
                                    lambda dt: self.dialog.dismiss(), 0.1
                                )
                                Clock.schedule_once(
                                    lambda dt: self._show_user_details(
                                        self.users_list[i]
                                    ),
                                    0.3,
                                )
    ...
    def debug_check_putitem_calls(self):
        ...
        if found_calls:
            Logger.warning(
                f"Found potential PutItem calls in these methods: {found_calls}"
            )
        else:
    ...
    def show_tab(self, tab_name):
        ...
        if tab_to_show not in self.tab_names:
            print(
                f"Warning: Tab '{tab_to_show}' not in available tabs: {self.tab_names}"
            )
    ...
    def refresh_data(self):
        ...
        if (
            hasattr(self, "folder_details_popup")
            and self.folder_details_popup
            and self.folder_details_popup.content
        ):
            ...
            if folder_title.startswith("Folder: "):
                ...
                Clock.schedule_once(
                    lambda dt: asyncio.run_coroutine_threadsafe(
                        self._show_folder_details(folder), app.loop
                    ),
                    0.1,
                )
    ...
    def _update_storage_display(self, stats):
        ...
        if hasattr(self.ids, "storage_label"):
            self.ids.storage_label.text = (
                f"{stats.get('total_size_gb', 0):.1f} GB / 50 GB"
            )
    ...
    def _update_mock_storage_stats(self):
        ...
        mock_stats = {"total_size_gb": 2.5, "usage_percentage": 5, "total_files": 15}
        ...
        try:
            if self.user_manager:
                ...
                if users:
                    ...
                    Logger.info(
                        f"Successfully loaded {len(self.users_list)} users from DynamoDB"
                    )
                else:
                    Logger.warning(
                        "No users found in DynamoDB, checking if we need to create default admin"
                    )
                    ...
                    Logger.info(
                        f"Loaded {len(self.users_list)} users after admin check"
                    )
            else:
    ...
    async def _ensure_admin_user(self):
        ...
        try:
            ...
            if not admin_username or not admin_password:
                Logger.warning(
                    "Admin credentials not configured, cannot create default admin"
                )
                return
    ...
    def _update_users_list(self):
        ...
        for user in self.users_list:
            ...
            item = MDListItem(
                ...
                MDListItemHeadlineText(text=f"{username} ({role.capitalize()})"),
                MDListItemSupportingText(
            ...
            details_btn = MDButton(
                style="text", on_release=lambda x, u=user: self._show_user_details(u)
            )
            ...
            status_btn = MDButton(
                style="text",
                on_release=lambda x, u=user: self._run_async_toggle(
                    u["username"], u["status"]
                ),
            )
            status_btn.add_widget(
                MDButtonText(text="DISABLE" if status == "active" else "ENABLE")
            )
    ...
    async def _toggle_user_status(self, username: str, current_status: str):
        ...
        try:
            ...
            if self.user_manager:
                result = await self.user_manager.update_user_status(
                    username, new_status
                )
                if result["success"]:
                    ...
                    self.show_snackbar(
                        f"User {username} status updated to {new_status}"
                    )
                else:
                    self.show_snackbar(
                        f"Failed to update user status: {result.get('error', 'Unknown error')}"
                    )
            else:
        ...
        try:
            ...
            if result and result.get("success"):
                ...
                if new_status:
                    ...
                    Logger.info(
                        f"Status update successful for {username}: {new_status}"
                    )
    ...
    def _handle_edit_user(
        self, username, role, access_level, folder_access, popup=None
    ):
        ...
        folder_list = [
            folder.strip() for folder in folder_access.split(",") if folder.strip()
        ]
        ...
        asyncio.run_coroutine_threadsafe(
            self._async_update_user(username, updates), app.loop
        )
        ...
        try:
            ...
            for field_id, label, value in fields:
                field = CustomTextField(
                    hint_text=label, text=str(value), size_hint_y=None, height=dp(48)
                )
            ...
            # Cancel button
            cancel_btn = MDButton(
                style="text", on_release=lambda x: self.edit_dialog.dismiss()
            )
    ...
    def _execute_save_operation(self, username):
        ...
        try:
            ...
            future = asyncio.run_coroutine_threadsafe(
                self._handle_save_edit(username), app.loop
            )
    ...
    async def _handle_save_edit(self, username):
        ...
        try:
            ...
            if updates["access_level"] not in ["pull", "push", "both", "full"]:
                return {
                    "success": False,
                    "error": "Access level must be 'pull', 'push', 'both', or 'full'",
                }
            ...
            while retry_count < max_retries:
                try:
                    ...
                    Logger.info(
                        f"Update attempt {retry_count + 1} for user {username} with data: {updates}"
                    )

                    # Use DynamoManager's update_user method
                    updated_user = await self.dynamo_manager.update_user(
                        username, updates
                    )
                    ...
                    if (
                        "ValidationException" in last_error
                        and "Missing the key username" in last_error
                    ):
                        Logger.error(
                            "Username key missing error detected, trying alternative approach"
                        )
                        try:
                            # Try direct update with explicit key
                            result = await self._update_user_directly_in_dynamo(
                                username, updates
                            )
                            if result:
                                Logger.info(
                                    f"Direct update successful for user {username}"
                                )
            ...
            Logger.error(
                f"All update attempts failed after {max_retries} retries. Last error: {last_error}"
            )
            return {
                "success": False,
    ...
    def _show_user_details(self, user):
        ...
        try:
            ...
            fields_container = MDBoxLayout(
                orientation="vertical", spacing=dp(20), size_hint_y=None, height=dp(300)
            )
            ...
            for field in field_data:
                ...
                label = MDLabel(
                    text=field["label"], bold=True, size_hint_x=0.3, halign="left"
                )

                value = MDLabel(text=field["value"], size_hint_x=0.7, halign="left")
            ...
            folder_value = MDLabel(
                text=folder_text, size_hint_y=None, height=dp(80), halign="left"
            )
    ...
    def _show_update_role_dialog(self, user):
        ...
        # Create content box
        content = MDBoxLayout(
            orientation="vertical", spacing=dp(16), padding=dp(20), adaptive_height=True
        )
        ...
        self.role_input = MDTextField(
            hint_text="New Role", mode="outlined", size_hint_y=None, height=dp(48)
        )
        ...
        update_button = MDButton(
            style="filled",
            on_release=lambda x: self._handle_update_role(
                user["username"], self.role_input.text
            ),
        )
    ...
    def _show_folder_selector(self, user):
        ...
        try:
            ...
            def selection_callback(selected_folders):
                Logger.info(
                    f"SELECTION CALLBACK CALLED with folders: {selected_folders}"
                )
    ...
    def _handle_folder_selection(self, username, selected_folders):
        ...
        try:
            Logger.info(
                f"Processing folder selection for {username}: {selected_folders}"
            )
            ...
            def update_in_thread():
                try:
                    ...
                    Clock.schedule_once(
                        lambda dt: self._handle_update_result(
                            result, username, selected_folders
                        ),
                        0,
                    )
                except Exception as e:
                    ...
                    Clock.schedule_once(
                        lambda dt: self.show_snackbar(f"Error: {str(e)}"), 0
                    )
    ...
    def _handle_update_completion(self, future):
        ...
        try:
            ...
            if result and result.get("success"):
                # Show success message
                self.show_snackbar(
                    f"Folder access updated successfully for {result.get('username')}"
                )
    ...
    def _handle_update_permissions(self, username, folders_text, access_level):
        ...
        try:
            Logger.info(
                f"Updating permissions for user: {username} with access level: {access_level}"
            )
            ...
            future = asyncio.run_coroutine_threadsafe(
                self._update_user_directly_in_dynamo(username, updates), app.loop
            )
    ...
    async def _load_folders(self):
        ...
        try:
            ...
            response = await asyncio.to_thread(
                s3_client.list_objects_v2, Bucket=bucket_name, Delimiter="/"
            )
        ...
        try:
            ...
            if hasattr(self, "dynamo_manager") and self.dynamo_manager:
                try:
                    ...
                    for item in response.get("Items", []):
                        ...
                        if not any(
                            u["username"] == username for u in users_with_access
                        ):
                            users_with_access.append(
                                {
                                    "username": username,
            ...
            if not users_with_access:
                ...
                for user in users_with_access:
                    ...
                    users_list.add_widget(
                        MDListItem(MDListItemHeadlineText(text=display_text))
                    )
            ...
            close_button = MDButton(
                style="text", on_release=lambda x: self._dismiss_folder_details()
            )
    ...
    def _show_manage_folder_access_popup(self, folder):
        ...
        self.access_username_input = MDTextField(
            hint_text="Enter Username", mode="outlined", size_hint_y=None, height=dp(48)
        )
        ...
        # Cancel button
        cancel_button = MDButton(
            style="text", on_release=lambda x: self._dismiss_access_popup()
        )
        cancel_button.add_widget(MDButtonText(text="CANCEL"))
        buttons.add_widget(cancel_button)
    ...
    def _handle_grant_access(self, username, folder, access_level="pull"):
        ...
        if access_level not in ["pull", "push", "full"]:
            self.show_snackbar(
                "Invalid access level. Must be 'pull', 'push', or 'full'"
            )
            return
        ...
        async def update_permissions():
            try:
                ...
                if not user:
                    ...
                    Clock.schedule_once(
                        lambda dt: self.show_snackbar(f"User {username} not found"), 0
                    )
                    return False
                ...
                try:
                    ...
                    if hasattr(self, "dynamo_manager") and self.dynamo_manager:
                        try:
                            ...
                            existing_perm = await asyncio.to_thread(
                                ...
                                Key={"username": username, "folder_path": folder},
                            )
                            ...
                            if "Item" in existing_perm:
                                ...
                                Logger.info(
                                    f"Updated existing permission record for {username}"
                                )
                            else:
                                ...
                                Logger.info(
                                    f"Created new permission record for {username}"
                                )
                            ...
                            if "Item" not in verify_perm:
                                raise Exception(
                                    "Permission record not found after saving"
                                )

                        except Exception as perm_error:
                            Logger.error(
                                f"Error updating permissions table: {str(perm_error)}"
                            )
                            raise perm_error
                    ...
                    if folder not in folder_access:
                        ...
                        Logger.info(
                            f"Updating user {username} with new folder access: {folder_access}"
                        )
                        ...
                        if not result.get("success"):
                            ...
                            Logger.error(
                                f"Failed to update user in DynamoDB: {error_msg}"
                            )
                            raise Exception(f"Failed to update user: {error_msg}")

                        Logger.info(
                            f"Successfully updated user {username} in DynamoDB users table"
                        )
                    ...
                    Clock.schedule_once(
                        lambda dt: self.show_snackbar(
                            f"Access granted for {username} to {folder}"
                        ),
                        0,
                    )
                    return True
                    ...
                    Clock.schedule_once(
                        lambda dt: self.show_snackbar(f"Error: {str(update_error)}"), 0
                    )
                    return False
        ...
        future.add_done_callback(
            lambda f: Clock.schedule_once(lambda dt: self.refresh_data(), 0)
        )
            ...
            try:
                ...
                await asyncio.to_thread(
                    ...
                    ExpressionAttributeValues={
                        ...
                        ":revoked_by": getattr(app, "current_user", {}).get(
                            "username", "system"
                        ),
                    },
                )
                ...
                if user:
                    ...
                    if folder in folder_access:
                        ...
                        self.show_snackbar(
                            f"Access revoked for {username} from {folder}"
                        )
                        return True
                    else:
                        self.show_snackbar(
                            f"User {username} does not have access to {folder}"
                        )
                        return False
                else:
                    self.show_snackbar(f"User {username} not found")
    ...
    async def _create_user_directly_in_dynamo(self, user_data):
        ...
        try:
            Logger.info(
                f"Attempting direct DynamoDB user creation for: {user_data['username']}"
            )
            ...
            if "Item" in existing_user:
                Logger.warning(
                    f"User {user_data['username']} already exists in DynamoDB"
                )
            ...
            await asyncio.to_thread(users_table.put_item, Item=user_item)
            ...
            if "Item" in verify_response:
                Logger.info(
                    f"User {user_data['username']} successfully created directly in DynamoDB"
                )
                ...
                Logger.error(
                    f"User {user_data['username']} not found after creation attempt"
                )
                return {
                    "success": False,
                    "error": "User creation failed - not found after creation",
                }
    ...
    def show_add_user_dialog(self):
        ...
        # Add spacer
        spacer = MDBoxLayout(size_hint_y=None, height=dp(20))  # Space before help text
        ...
        # Add another spacer
        spacer2 = MDBoxLayout(size_hint_y=None, height=dp(20))  # Space before buttons
    ...
    def _handle_add_user(self, username, password, role, access_level):
        ...
        Logger.info(
            f"_handle_add_user called with username: {username}, role: {role}, access_level: {access_level}"
        )
    ...
    def logout(self):
        ...
        try:
            ...
            try:
                ...
                Clock.schedule_once(
                    lambda dt: setattr(app.root, "current", "login"), 0.1
                )
        ...
        try:
            ...
            if access_level in ["push", "full", "both"]:
                upload_button = MDButton(
                    style="filled", size_hint=(None, None), size=(dp(120), dp(48))
                )
                upload_button.add_widget(MDButtonText(text="Upload File"))
                upload_button.bind(
                    on_release=lambda x: self._show_upload_dialog(folder_path)
                )
            ...
            if access_level in ["push", "full", "both"]:
                subfolder_button = MDButton(
                    style="filled", size_hint=(None, None), size=(dp(140), dp(48))
                )
                subfolder_button.add_widget(MDButtonText(text="New Subfolder"))
                subfolder_button.bind(
                    on_release=lambda x: self._show_create_subfolder_dialog(folder_path)
                )
            ...
            try:
                ...
                if folder_path != "/":
                    ...
                    parent_item = MDListItem(
                        ...
                        MDListItemHeadlineText(text=f".. (Parent Directory)"),
                    )
                    parent_item.bind(
                        on_release=lambda x: self._show_folder_contents(parent_path)
                    )
                ...
                if "CommonPrefixes" in response:
                    for prefix in response["CommonPrefixes"]:
                        ...
                        folder_item.bind(
                            on_release=lambda x, p=prefix[
                                "Prefix"
                            ]: self._show_folder_contents(p)
                        )
                ...
                for item in response.get("Contents", []):
                    ...
                    actions_menu = [
                        [
                            "Download",
                            "download",
                            lambda x: self._handle_download_file(item["Key"]),
                        ]
                    ]
                    ...
                    if access_level in ["push", "full", "both"]:
                        actions_menu.append(
                            [
                                "Delete",
                                "delete",
                                lambda x: self._handle_delete_file(item["Key"]),
                            ]
                        )

                    file_item.bind(
                        on_release=lambda x, f=item["Key"]: self._show_file_actions(
                            f, actions_menu
                        )
                    )
                    files_list.add_widget(file_item)

                if not response.get("Contents", []) and not response.get(
                    "CommonPrefixes", []
                ):
                    empty_item = MDListItem(
                        MDListItemHeadlineText(text="Folder is empty")
                    )
                    files_list.add_widget(empty_item)

            except Exception as e:
                Logger.error(f"Error listing folder contents: {str(e)}")
                error_item = MDListItem(MDListItemHeadlineText(text=f"Error: {str(e)}"))
    ...
    def _show_file_actions(self, file_path, actions):
        ...
        buttons = MDBoxLayout(
            orientation="vertical", spacing=dp(8), adaptive_height=True
        )
    ...
    def _handle_download_file(self, file_path):
        ...
        try:
            ...
            save_path = filedialog.asksaveasfilename(
                defaultextension="", initialfile=file_path.split("/")[-1]
            )
            import os
    ...
    def _handle_delete_file(self, file_path):
        ...
        try:
            ...
            # Delete file
            s3_client.delete_object(Bucket=self.s3_helper.bucket_name, Key=file_path)
            ...
            if self.audit_logger:
                asyncio.create_task(
                    self.audit_logger.log_action(
                        ...
                        {"file_path": file_path},
                        user_id=(
                            self.user_manager.current_user.get("user_id")
                            if self.user_manager
                            else None
                        ),
                    )
    ...
    def _show_upload_dialog(self, folder_path):
        ...
        try:
            ...
            for file_path in file_paths:
                ...
                file_name = os.path.basename(file_path)
                files_info.append(
                    {"path": file_path, "name": file_name, "size": file_size}
                )
            ...
            self.show_snackbar(
                f"Selected {file_count} file{'s' if file_count > 1 else ''} for upload"
            )

            # Create progress dialog
            self.upload_progress = EnhancedUploadDialog(
                files_info=files_info, on_cancel=self._cancel_upload
            )
            ...
            def upload_thread():
                try:
                    ...
                    Clock.schedule_once(
                        lambda dt: self.show_snackbar(f"Upload error: {str(e)}"), 0
                    )
    ...
    def _upload_files(self, files_info, folder_path):
        ...
        try:
            ...
            )  # Replace with your actual bucket name if needed
            ...
            for index, file_info in enumerate(files_info):
                    ...
                    Clock.schedule_once(
                        lambda dt: (
                            self._update_upload_progress(
                                index, bytes_transferred, file_size
                            )
                ...
                s3_client.upload_file(
                    local_path, bucket_name, s3_key, Callback=progress_callback
                )
            ...
            Clock.schedule_once(
                lambda dt: self._refresh_folder_contents(folder_path), 2
            )
            ...
            if "Upload cancelled by user" in str(e):
                # Handle cancellation
                Clock.schedule_once(
                    lambda dt: self.show_snackbar("Upload cancelled"), 0
                )
            else:
                # Handle other errors
                Clock.schedule_once(
    ...
    def _update_upload_progress(self, file_index, bytes_transferred, file_size):
        ...
        if hasattr(self, "upload_progress") and self.upload_progress:
            self.upload_progress.update_progress(
                file_index, bytes_transferred, file_size
            )
    ...
    def _show_create_subfolder_dialog(self, parent_folder):
        ...
        self.subfolder_name_input = MDTextField(
            hint_text="Subfolder Name", mode="outlined", size_hint_y=None, height=dp(48)
        )
        ...
        # Cancel button
        cancel_button = MDButton(
            style="text", on_release=lambda x: self.subfolder_popup.dismiss()
        )
        cancel_button.add_widget(MDButtonText(text="CANCEL"))
        buttons.add_widget(cancel_button)
    ...
    def _filter_users(self, search_text):
        ...
        if not search_text:
            ...
            self.users_list = [
                ...
                if search_text in user.get("username", "").lower()
    ...
    def _confirm_delete_user(self):
        ...
        try:
            if hasattr(self, "user_to_delete") and self.user_to_delete:
                ...
                self.users_list = [
                    u for u in self.users_list if u.get("username") != username
                ]
                Clock.schedule_once(lambda dt: self._update_users_list(), 0)

                # Run deletion in background with callback
                future = asyncio.run_coroutine_threadsafe(
                    self._handle_delete_user(self.user_to_delete), app.loop
                )
    ...
    def _force_reload_users(self, *args):
        ...
        try:
            ...
            future = asyncio.run_coroutine_threadsafe(
                self._reload_users_from_dynamo(), app.loop
            )

            # Add callback to update UI when complete
            future.add_done_callback(
                lambda f: Clock.schedule_once(lambda dt: self._update_users_list(), 0)
            )
    ...
    async def _handle_delete_user(self, user):
        ...
        try:
            ...
            # If we get here, all retries failed
            Logger.error(
                f"All delete attempts failed after {max_retries} retries. Last error: {last_error}"
            )
            return {
                "success": False,
                "error": f"Failed after {max_retries} attempts: {last_error}",
